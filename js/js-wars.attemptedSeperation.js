...
app = {	
	...
	settings:{
		...
		movable:{
			foot: [ 'plain', 'river', 'mountain', 'wood', 'road' ],
			wheels: [ 'plain', 'wood', 'road' ],
			flight: [ 'plain', 'river', 'mountain', 'wood', 'road', 'water' ],
			boat: ['water']
		}
	...
	game: function () {
		...
		calculate = {
			...
			offset: function (off, orig) {
				...
				if ( app.inArray( off.type, orig.movable )){
		...
		return {
			...
			interact: function (type) {
				...
				// if there is an object being hovered over, or hovered is undefined ( meaning input is possibly from type input rather then hud )
					...
					if( select.element( typ, intInd ) && temp.selectedUnit ){
						if ( movement.move( 'unit', temp.selectedUnit.ind )){
							window.requestAnimationFrame(app.animateUnit);
						}						
					}
				}
			...
			move: function () {
				...
				if ( now > delay ){
					...
					// if there has been a change then animate the cursor to reflect it
					window.requestAnimationFrame(app.animateCursor);
				}	
				return this;
			}
...
app.map = {
	...
	building: [
		//{ x:0, y:5, type:'hq', player:1, color: 'red' , def:4},
		//{ x:15, y:5, type:'hq', player:2, color: 'blue' , def:4},
		{ x:0,y:4, type:'base', player:1,color:'red', def:4},
		{ x:15,y:4, type:'base', player:2, color:'blue', def:4}
	],
	unit:[
		{ x:2, y:5, type:'infantry', obsticle:'unit', movement:3, movable:[ 'plain', 'river', 'mountain', 'wood' ], player: 1, health:10, ammo:10 ,fuel:100, id: 1 }
	]
};
...
app.animateBuildings = function () {
	app.buildingCanvas.setAnimations(app.drawBuildings).render(app.animateBuildings);
};

app.animateUnit = function () {
	app.unitCanvas.setAnimations(app.drawUnits).render(app.animateUnit);
};
...
/* --------------------------------------------------------------------------------------------------------*\
	...
	Main game loop: runs game logic which controls animation refresh calls, coordinates, displays, etc..
...
/* -------------------------------------------------------------------------------------------------------------*\
	...
	Run, starts the main game loop and calls all the animate functions, putting everything together on the screen
